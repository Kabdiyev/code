...
def update_shanyrak_photos(
    ...
    for file in files:
        if not is_image(file.file.read()):
            raise HTTPException(
                status_code=400, detail=f"File {file.filename} is not image"
            )
        url = svc.s3_service.upload_file(file.file, file.filename)
        if url is None:
            raise HTTPException(
                status_code=500, detail=f"File {file.filename} not uploaded"
            )
    ...
    if update_result.acknowledged:
        return media_urls
    raise HTTPException(
        status_code=404, detail=f"Error occured while updating shanyrak {shanyrak_id}"
    )
...
def update_shanyrak_photos(
    ...
    shanyrak = svc.repository.get_shanyrak_by_id(
        shanyrak_id=shanyrak_id, user_id=jwt_data.user_id
    )
    ...
    if not update_result.acknowledged:
        raise HTTPException(
            status_code=404,
            detail=f"Error occured while deleting photos from shanyrak {shanyrak_id}",
        )
    return to_delete
    # if update_result.modified_count == 1:
    #     return Response(status_code=200)
